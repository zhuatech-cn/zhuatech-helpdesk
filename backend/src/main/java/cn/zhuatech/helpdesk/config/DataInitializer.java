...
package cn.zhuatech.helpdesk.config;
import cn.zhuatech.helpdesk.domain.DomainCatalog;
import cn.zhuatech.helpdesk.model.*;
import cn.zhuatech.helpdesk.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "HELPDESK-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="HELPDESK-DEMO-"+String.format("%03d",sequence);
